...
const main = async () => {
  ...
  //Signing with ACCOUNT1_PRIVATE_KEY
  const signedTxn = await transaction.sign(PrivateKey.fromString(ACCOUNT1_PRIVATE_KEY));

  //Signing with ACCOUNT2_PRIVATE_KEY
  const multiSignedTxn = await signedTxn.sign(
    PrivateKey.fromString(ACCOUNT2_PRIVATE_KEY)
  );
